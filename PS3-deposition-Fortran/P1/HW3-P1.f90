program P1
   ...
   implicit none
    ...
    integer, parameter :: exp_steps = 2000000
    ...
    integer :: color = 1
    integer :: i, j, k, fu        ! int for loop and file stuff
    ...
    character(len=*), parameter :: VAR_FILE = 'var-P1.txt'  ! log of var for ploting
   ...
   open (action='write', file=OUT_FILE, newunit=fu, status='replace')
    ...
    sat_index = 18       ! Based on previous results
...
function find_dest(j_in)      ! Finding the particle's destination
    ...
    if (h(j_l) < h(j_in)) then
        ...
        if (h(j_l) < h(j_r)) then
            ...
            if (get_rpos(2) == 1.0) then     ! Selecting a random position between two
                find_dest = j_r
            else
                find_dest = j_l
            end if
